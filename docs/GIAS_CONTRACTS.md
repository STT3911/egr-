...
2. Первичный обход идёт по временным окнам `dtCreateFrom/dtCreateTo`.
   Окно, страница и фиксированная конечная дата обхода хранятся в
   `gias_contract_sync_state`, поэтому после сбоя или перезапуска загрузка
   продолжается с последней завершённой страницы.
3. Если окно содержит больше безопасных 90 страниц, оно автоматически
   делится пополам. Это обходит ограничение поиска GIAS примерно в 10 000
   результатов и не позволяет пустой промежуточной странице ошибочно
   завершить весь первичный обход.
4. Очередь деталей выбирает договоры со статусом `pending` или `error` и
   получает полную карточку.
5. Позиции договора нормализуются в `gias_contract_positions`.
6. Исходные ответы без потерь сохраняются в `raw_summary` и `raw_detail`.
...
7. Заказчик и поставщик связываются с `egr_companies` отдельными внешними
   ключами.
